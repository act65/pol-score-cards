...
| 2026-09-25 | **Forthrightness WITHHELD, Divination released.** The `/party` page showed Forthrightness at 62 for government caucuses and 19 for opposition ones — a 44-point gap where no other attribute exceeds 16. Two causes. (1) A pairing bug: 43 of 74 scored MPs rest on ≤10 Q/A pairs and average 24 against 67 for the 31 with real volume, and in those rows the "answer" is the MP's own next supplementary question (`asked_by` == the scored MP). (2) The one that decides it: **only the executive answers oral questions**. Of the 31 MPs with a usable sample, 31 are government — National 22, ACT 5, NZ First 4, zero opposition. Fixing (1) cannot fix (2), so on a 133-card grid the attribute reads as a bonus for being in government. It stays extracted: it is a good measure *of ministers*, and an executive-accountability view is worth building. Divination returns in its place — the 2026-09-24 decision used the **resolved-only** count (29 MPs); with `--use_prior` it covers 127 of 133 (95%) at a median of 10 predictions each, its government/opposition gap is −1, and its mean |r| against the other five is **0.17**, the most independent of the seven. Cards show **six**: Veracity, Divination, Focus, Civility, Rigor, Specificity — two of them (Veracity, Divination) still marked unverified. |
