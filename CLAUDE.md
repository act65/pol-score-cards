...
pytest                                 # data/tests + attribute-extraction/tests + game
...
## Notes
- **Repo layout**: tests live in `data/tests/` and `attribute-extraction/tests/`, each with a `conftest.py` that puts the sibling modules on the path (the pipelines are run as scripts from their own directories, so they import each other by bare name). `pytest.ini` at the root runs everything.
