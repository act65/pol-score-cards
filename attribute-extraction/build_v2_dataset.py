...
def run(scores="hansard_scores_3mo.jsonl", out="site_data_v2",
    ...
    per_pair = collections.defaultdict(list)        # (mid, attr) -> [score]
    ...
    for rec in rows:
        date = rec.get("date", "")
        dates.add(date)
        url = f"https://hansard.parliament.nz/hansard-transcript/{date}"
    ...
    manifest = {
        ...
        "source": corpus_label + " (Hansard only)",
